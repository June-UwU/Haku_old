...
Graphics::Graphics(HWND Handle)
{
	...
	HAKU_INFO_QUEUE_CHECK_DUMP(
		CreateDXGIFactory1(__uuidof(IDXGIFactory), reinterpret_cast<void**>(_Factory.GetAddressOf())))
...
#endif
	HAKU_INFO_QUEUE_CHECK_DUMP(D3D11CreateDeviceAndSwapChain(
		_Adapter.Get(),
	...
	HAKU_INFO_QUEUE_CHECK_DUMP(
		_Device->CreateRenderTargetView(_RenderingBackBuffer.Get(), nullptr, _RenderTarget.GetAddressOf()))
	...
	HAKU_INFO_QUEUE_CHECK_DUMP(
		_Device->CreateTexture2D(&DepthStencilBufferDesc, nullptr, _DepthStencilBuffer.GetAddressOf()))
...
void Graphics::Tinkering(float ThetaZ, float Translation)
{
	...
	HAKU_INFO_QUEUE_CHECK_DUMP(D3DCompileFromFile(
	...
	Rotation Matrix{
		...
		DirectX::XMMatrixTranspose(
			...
			DirectX::XMMatrixPerspectiveLH(1.0, -ClientHeight / ClientWidth, 0.5f, 10.00f))
	...
	HAKU_INFO_QUEUE_CHECK_DUMP(_Device->CreateInputLayout(
		VertexInputDesc,
